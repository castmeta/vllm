...
class MultiModalDiskCache:
    ...
    def __init__(
        ...
        *,
        ...
        logger.debug(
            "MultiModalDiskCache initialised at '%s' (max_items=%d, existing_items=%d)",
            self._root,
            self._max_items,
            len(self._index),
        )
    ...
    def load_prompt_updates(self, mm_hash: str) -> "list[ResolvedPromptUpdate]":
    ...
    def save_item_async(
        ...
        If a hash_prefix filter is set and the mm_hash does not start with it,
        the item is silently skipped.
        """
        # Filter by hash prefix if configured
        if self._hash_prefix is not None and not mm_hash.startswith(self._hash_prefix):
            return
